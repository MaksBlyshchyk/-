...
public class CandidatesController(ApplicationDbContext context) : Controller
{
    ...
    public async Task<IActionResult> Create([Bind("FullName,Email,Phone,Skills,ExperienceYears")] Candidate candidate)
    {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(int id, [Bind("Id,FullName,Email,Phone,Skills,ExperienceYears,CreatedAt")] Candidate candidate)
    {
        if (id != candidate.Id)
        {
            return NotFound();
