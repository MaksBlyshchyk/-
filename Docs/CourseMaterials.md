...
Система має підтримувати базу резюме із завантаженням файлів, етапи відбору з коментарями рекрутера, відгуки інтерв'юерів, статуси вакансій, календар співбесід, `.ics` export, email-сповіщення, рольовий доступ через ASP.NET Identity і демонстраційні облікові записи для перевірки.
...
- Authentication layer: `AccountController`, ASP.NET Identity і `IdentityRecruiterSyncService` реалізують cookie authentication, ролі та хешування паролів.
- Notification layer: `EmailNotificationService` надсилає email через SMTP або зберігає повідомлення в локальний `EmailOutbox`, якщо SMTP не налаштований.
...
| `Applications` | Пов'язує кандидата з вакансією, показує етап відбору та коментар рекрутера щодо наступного кроку. |
...
3. У розділі "Кандидати" показати пошук, фільтр за досвідом, завантаження або відкриття резюме, деталі кандидата із середнім soft skills score та CSV export.
4. У розділі "Вакансії" показати фільтр за статусом, у "Заявках" - фільтр за етапом відбору та коментар рекрутера, у "Співбесідах" - фільтр за типом, результатом, календар співбесід, `.ics` export і створення email-сповіщення.
...
5. У розділі "Кандидати" додати або знайти кандидата за ПІБ, email, навичками чи досвідом, завантажити файл резюме, а також експортувати список у CSV.
6. У розділі "Вакансії" додати вакансію або відфільтрувати список за статусом.
7. У розділі "Заявки" відстежити етапи відбору кандидатів і додати коментар рекрутера.
8. У розділі "Співбесіди" переглянути історію, фільтри, календар співбесід і експорт `.ics`.
