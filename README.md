...
Користувачі зберігаються в таблиці `Recruiters`. `DemoUserService` перевіряє логін і пароль з БД, тобто це навчальна демо-авторизація без ASP.NET Identity.
...
4. Відкрити основні сторінки:
   ...
   - `/Recruiters`
   - `/Home/About`
5. Для кожної сутності перевірити Index, Create, Edit, Details, Delete.
6. Увійти як `recruiter` і перевірити, що Feedbacks, Soft Skills і Recruiters недоступні.
7. Увійти як `interviewer` і перевірити доступ до Interviews, Feedbacks і Soft Skills.
...
- Експорт кандидатів у CSV/Excel.
